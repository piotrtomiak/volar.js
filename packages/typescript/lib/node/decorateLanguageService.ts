...
export function decorateLanguageService(
	...
	languageService.getNavigationTree = filePath => {
		...
		if ((serviceScript && sourceScript) || sourceScript?.nonTs) {
	...
	languageService.getOutliningSpans = filePath => {
		const fileName = filePath.replace(windowsPathReg, '/');
		const [serviceScript, sourceScript] = getServiceScript(language, fileName);
		if (serviceScript || sourceScript?.nonTs) {
			return [];
		}
		else {
	...
	languageService.getFormattingEditsForDocument = (filePath, options) => {
		...
		if (serviceScript) {
			...
			return edits
				.map(edit => takeIfSameName(fileName, transformTextChange(sourceScript, map, edit, isFormattingEnabled)))
				.filter(notEmpty);
		}
		else {
		...
		if (serviceScript) {
			...
			if (generatePosition !== undefined) {
				...
				if (info) {
					return {
						ranges: info.ranges
							.map(span => takeIfSameName(fileName, transformTextSpan(sourceScript, map, span, isLinkedEditingEnabled)))
							.filter(notEmpty),
			...
			.map(call => {
				...
				const fromSpans = call.fromSpans
					.map(span => serviceScript
						? takeIfSameName(fileName, transformTextSpan(sourceScript, map, span, isCallHierarchyEnabled))
						: span
					)
					.filter(notEmpty);
				return {
	...
	languageService.getQuickInfoAtPosition = (filePath, position) => {
		...
		if (serviceScript) {
			...
			for (const [generatePosition, mapping] of toGeneratedOffsets(sourceScript, map, position)) {
				...
				if (info) {
					const textSpan = takeIfSameName(fileName, transformTextSpan(sourceScript, map, info.textSpan, isHoverEnabled));
					if (textSpan) {
						infos.push({
							...info,
							textSpan: textSpan,
	...
	languageService.getSignatureHelpItems = (filePath, position, options) => {
		...
		if (serviceScript) {
			...
			if (generatePosition !== undefined) {
				...
				if (result) {
					const applicableSpan = takeIfSameName(fileName, transformTextSpan(sourceScript, map, result.applicableSpan, isSignatureHelpEnabled));
					if (applicableSpan) {
						return {
							...result,
	...
	languageService.getRenameInfo = (filePath, position, options) => {
		...
		if (serviceScript) {
			...
			for (const [generateOffset, mapping] of toGeneratedOffsets(sourceScript, map, position)) {
				...
				if (info.canRename) {
					const span = takeIfSameName(fileName, transformTextSpan(sourceScript, map, info.triggerSpan, isRenameEnabled));
					if (span) {
	...
	languageService.getEncodedSemanticClassifications = (filePath, span, format) => {
		...
		if (serviceScript) {
			...
			for (let i = 0; i < result.spans.length; i += 3) {
				const sourceStart = takeIfSameName(fileName, toSourceOffset(sourceScript, map, result.spans[i], isSemanticTokensEnabled));
				const sourceEnd = takeIfSameName(fileName, toSourceOffset(sourceScript, map, result.spans[i] + result.spans[i + 1], isSemanticTokensEnabled));
		...
		if (serviceScript) {
			...
			for (const [generatedOffset, mapping] of toGeneratedOffsets(sourceScript, map, position)) {
				...
				for (const entry of result.entries) {
					entry.replacementSpan = entry.replacementSpan && takeIfSameName(fileName, transformTextSpan(sourceScript, map, entry.replacementSpan, isCompletionEnabled));
				}
				result.optionalReplacementSpan = result.optionalReplacementSpan
					&& takeIfSameName(fileName, transformTextSpan(sourceScript, map, result.optionalReplacementSpan, isCompletionEnabled));
	...
	languageService.provideInlayHints = (filePath, span, preferences) => {
		...
		if (serviceScript) {
			...
			for (const hint of result) {
				const sourcePosition = takeIfSameName(fileName, toSourceOffset(sourceScript, map, hint.position, isInlayHintsEnabled));
		...
		if (serviceScript) {
			for (const [generatedOffset, mapping] of map.getGeneratedOffsets(position)) {
				if (filter(mapping.data)) {
					process(sourceScript.id, generatedOffset + sourceScript.snapshot.getLength());
				}
			}
		}
		else {
